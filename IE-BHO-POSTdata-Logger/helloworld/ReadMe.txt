========================================================================
    活动模板库：helloworld 项目概述
========================================================================

应用程序向导已为您创建了此 helloworld 项目，作为编写动态链接库 (DLL) 的起点。
...
helloworld.idl
    ...
    此文件将由 MIDL 编译器处理，用于生成：
        C++ 接口定义和 GUID 声明 (helloworld.h)
        GUID 定义                                (helloworld_i.c)
        类型库                                  (helloworld.tlb)
        封送处理代码                                 （helloworld_p.c 和 dlldata.c）

helloworld.h
    此文件包含 helloworld.idl 中定义的项目的 C++ 接口定义和 GUID 声明。它将在编译过程中由 MIDL 重新生成。
...
helloworldps.vcxproj
	...
	此过程生成 dlldata.c、helloworld_i.c 和 helloworld_p.c，这些是生成代理/存根 (stub) DLL 所必需的。
